...
func testTagToManyWidgetTags(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TagSlice{&a}
	if err = a.L.LoadWidgetTags(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTagToManyAddOpWidgetTags(t *testing.T) {
	var err error
...
var (
	tagDBTypes = map[string]string{`TagID`: `varchar`, `Name`: `varchar`, `Slug`: `varchar`, `Description`: `text`}
	_          = bytes.MinRead
)
